...
when he was developing a new \LaTeX{} class for typesetting ISO standards.
The development of the package has benefited from
...
\item[User-specified parameter values] Commands of the form 
    ...
    parameter \textit{P}. Note that these commands do \emph{not} affect
...
yourself with this as it is called automatically by the \file{layouts}
...
\texttt{cm}, \texttt{bp}, \texttt{dd}, or \texttt{cc}; an unknown unit is
...
Using \pixcom{drawmarginparstrue} results in representations of
...
\end{figure}
    ...
    When layouts are drawn with \verb|...diagram|\footnote{Or long-handedly
...
class page are the same as, or analogous to, those for a standard page 
layout.
...
on those for the current document. The parameters can all be individually
